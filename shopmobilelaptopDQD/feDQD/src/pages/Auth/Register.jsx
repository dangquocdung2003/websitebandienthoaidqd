...
export default function RegisterPage() {
  ...
  const strengthLabel = ["Rất yếu", "Yếu", "Trung bình", "Khá", "Mạnh"][strength];
  ...
  const jsonLd = useMemo(() => ({
    ...
    "potentialAction": {
      "@type": "RegisterAction",
      "target": typeof window !== "undefined" ? window.location.href : undefined
    }
  }), []);
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // Đăng ký xong → về /login, mang theo redirect nếu có
      const redirect = new URLSearchParams(location.search).get("redirect") || "/";
      nav(`/login?redirect=${encodeURIComponent(redirect)}`, { replace: true });
    } catch (err) {
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-10">
      ...
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        <div>
          <label htmlFor="name" className="mb-1 block text-sm font-medium">Họ và tên</label>
          <input
            id="name"
            type="text"
        ...
        <div>
          ...
          <div className="mt-2">
            ...
            <p className="mt-1 text-xs text-gray-600">Độ mạnh mật khẩu: <b>{strengthLabel}</b></p>
          </div>
        </div>

        <div>
          <label htmlFor="confirm" className="mb-1 block text-sm font-medium">Nhập lại mật khẩu</label>
          <div className="relative">
            <input
              id="confirm"
        ...
        <label className="flex items-start gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.agree}
            onChange={(e) => setForm(v => ({ ...v, agree: e.target.checked }))}
          />
          <span>
            Tôi đồng ý với <a href="/terms" className="underline">Điều khoản</a> và <a href="/privacy" className="underline">Chính sách</a>.
          </span>
        </label>
        ...
        <p className="text-center text-sm text-gray-600">
          Đã có tài khoản?{" "}
          <Link to="/login" className="text-gray-900 underline hover:opacity-80">Đăng nhập</Link>
        </p>
      </form>
    </main>
  );
